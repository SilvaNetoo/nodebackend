...
import { PessoasFisicas } from './rotas/pessoa-fisica';
import { PessoasJuridicas } from './rotas/pessoa-juridica';
...
class Servidor {
  ...
  private configuraRotas(): void {
    ...
    this.app.use('/pessoa-fisica', new PessoasFisicas().rotas());
    this.app.use('/pessoa-juridica', new PessoasJuridicas().rotas());
    this.app.use('/conta', new Conta().rotas());
  }
}
